...
const hasEnvCerts = process.env.KAFKA_CA_CERT && process.env.KAFKA_ACCESS_CERT && process.env.KAFKA_ACCESS_KEY;
const hasLocalCerts = fs.existsSync(caCertPath) && fs.existsSync(accessCertPath) && fs.existsSync(accessKeyPath);

if (hasEnvCerts || hasLocalCerts) {
  logger.info('Configuring Kafka for Secure Cloud Production Environment via mTLS');
  
  const ca = hasEnvCerts ? process.env.KAFKA_CA_CERT : fs.readFileSync(caCertPath, 'utf-8');
  const cert = hasEnvCerts ? process.env.KAFKA_ACCESS_CERT : fs.readFileSync(accessCertPath, 'utf-8');
  const key = hasEnvCerts ? process.env.KAFKA_ACCESS_KEY : fs.readFileSync(accessKeyPath, 'utf-8');
